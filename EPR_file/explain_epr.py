...
def get_data(token_cache_path, alignment_cache_path, phase, _lambda):
    with open(token_cache_path + phase + '_tokens.pkl', 'rb') as f:
        token_data = pickle.load(f)

    with open(alignment_cache_path + phase + '_alignment_' + str(_lambda) + '.pkl', 'rb') as f:
    ...
    with open(result_file, 'w') as of:
        with torch.no_grad():
            ...
            for item in tqdm(dataloader.get_datapoint(), total=len(dataloader)):
                ...
                premise = tokenizer.decode(item['s1_token']['input_ids'], skip_special_tokens=True)
                hypothesis = tokenizer.decode(item['s2_token']['input_ids'], skip_special_tokens=True)
...
if __name__ == '__main__':
    ...
    test_token_data, test_alignment_data = get_data(token_cache_path, alignment_cache_path, 'test',args._lambda)
    explain_dataloader = DataLoader(test_token_data, test_alignment_data, 'explain')
    results = do_explain(explain_dataloader, sbert, learnableToken, sp_indice, interactModel, induceFunction, device)



    df = pd.DataFrame(results)
    excel_file = "selected_results.xlsx"
    df.to_excel(excel_file, index=False)
    print(f"Selected results successfully written to {excel_file}")
